...
export class OnlinePaymentService {
  public mcid!: string;
  private mcc: string = '5732';
  private ccy: number = 418;
  private province: string = 'VTE';
  private country: string = 'LA';
  private debug: boolean = false;
  private lastToken: string = new Date().getTime() - 30 * 60 * 1000 + "0000";
  private expireTime: any;
  private tid: string | null = null;
  private shopCode: string | null = null;
  ...
  subscribe(params: any, onPaid: any) {
  ...
  pubnubSubscribe() {
    let channel = this.shopCode ? 'mcid-' + this.mcid + "-" + this.shopCode : 'uuid-' + this.mcid + "-" + this.uuid;
  ...
  private crcTable = [0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50a5,
  ...
  generateQRcode(data: QRcodeDto) {
    return this.http.post(Util.Api + ApiPath.opPayment + `/bank/bcelQR`, data);
  }

  updatedQRcode(data: QRcodeDto){
    return this.http.post(Util.Api + ApiPath.opPayment + `/bank/bcelQR/success`, data);
  }
